...
function GetTaobaoData() {
	...
	$.ajax({
		...
		success : function(html) {
			...
			$("#title").val(obj.item.title);
			$("#remark").val(obj.item.title);
			$("#price").val(obj.item.price);

			$("#taobao_tr").show();
			$("#taobao_pic_url").html(obj.item.pic_url);
			$("#thumb_preview").attr('src', obj.item.pic_url + '_200x200.jpg');
			$("#img").val(obj.item.pic_url);

			// 编辑器值
			editor1.insertHtml(obj.item.desc);
		}
	});
}
